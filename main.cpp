...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	{
		vertexBuffer.resource = CreateBufferResource(device, sizeof(Vector4) * 3);
		vertexBuffer.strideSize = sizeof(Vector4);
...

	{
		...
		while (msg.message != WM_QUIT) {
			...
			if (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) {
				...
				commandList->SetGraphicsRootConstantBufferView(0, materialBuffer.resource->GetGPUVirtualAddress());
				commandList->SetGraphicsRootConstantBufferView(1, wvpBuffer.resource->GetGPUVirtualAddress());
